...
Example usage:
    ...
    js = filter(lambda f: f.endswith('.js') and not f.startswith('AJS'),os.listdir('javascripts'))
    js = ['javascripts/'+f for f in js]
    html = filter(lambda f: f.endswith('.html'),os.listdir('templates'))
    html = ['templates/'+f for f in html]
    FILES = js + html
